"""Pipeline executor — pure function that walks a task through pipeline steps.
...
def _descend_to_leaf(
    ...
    return _action_for_step(step), PipelinePosition(path=current_path)
...
def _advance_from(
    ...
    if len(path) == 1:
        ...
        if next_idx >= len(pipeline):
            return None
        return _descend_to_leaf(pipeline, (), next_idx)
    ...
    if next_idx < len(parent_step.steps):
        ...
        return _descend_to_leaf(parent_step.steps, parent_path, next_idx)

    # End of this loop's steps — exit the loop and advance at the parent level
    return _advance_from(pipeline, parent_path)
...
def next_action(
    ...
    Args:
        pipeline: The full pipeline step list.
    ...
    """
    step = _resolve_step(pipeline, position.path)

    # --- No result: we're arriving at this step for the first time ---
    if result is None:
        return _action_for_step(step), position
    ...
    if isinstance(step, RoleStep):
        ...
        if override is not None:
            ...
            target_idx = _find_step_by_role(parent_steps, override)
            if target_idx is not None:
                return _descend_to_leaf(parent_steps, prefix, target_idx)

    if is_pass:
        # Advance to next step
        advanced = _advance_from(pipeline, position.path)
    ...
    restarted = _restart_loop(pipeline, position.path)
