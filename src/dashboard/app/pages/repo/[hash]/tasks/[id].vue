...
const { data: task, error } = useAsyncData<TaskDetail>(
  `task-${taskId.value}`,
  async () => {
    const result = await fetchTask(taskId.value, { repo: repoHash.value })
    markFetched()
    return result
  },
  { server: false },
...
const { data: pipelineSteps } = useAsyncData<PipelineStepInfo[]>(
  'pipeline-steps',
  async () => {
    const result = await fetchPipeline({ repo: repoHash.value })
    markFetched()
    return result
  },
...
async function loadPromptData(): Promise<void> {
  ...
  try {
    promptData.value = await fetchTaskPrompt(taskId.value, { repo: repoHash.value })
    markFetched()
  } catch {
    promptData.value = []
  }
...
const breadcrumbItems = computed(() => {
  ...
  if (task.value) {
    items.push({ label: task.value.spec_ref.split('@')[0], to: `/repo/${repoHash.value}/specs/${task.value.spec_ref.split('@')[0]}` })
...
async function handleRestart(): Promise<void> {
  ...
  try {
    await restartTask(task.value.id, { expected_round: task.value.round }, { repo: repoHash.value })
...
async function handleRetire(): Promise<void> {
  ...
  controlSuccess.value = null
  try {
    await retireTask(task.value.id, { expected_round: task.value.round }, { repo: repoHash.value })
...
async function handleForceClear(): Promise<void> {
  ...
  controlSuccess.value = null
  try {
    await forceClearTask(task.value.id, { expected_round: task.value.round }, { repo: repoHash.value })
...
<template>
  <div class="max-w-5xl mx-auto px-6 py-8">
    ...
    >
      ...
      <NuxtLink :to="`/repo/${repoHash}/activity`" class="mt-2 text-sm text-blue-600 dark:text-blue-400 hover:underline">
        Back to activity
      </NuxtLink>
    </div>
        ...
        <Transition name="tab" mode="out-in">
          ...
          <div v-if="activeTab === 'overview'" key="overview" class="space-y-6">
            ...
            <div class="rounded-lg bg-white dark:bg-gray-900 p-5 shadow-card dark:ring-1 dark:ring-white/[0.06] dark:shadow-none">
              ...
              <dl class="grid grid-cols-2 gap-x-8 gap-y-3 text-sm">
                ...
                <div>
                  ...
                  <dd class="mt-0.5">
                    <NuxtLink
                      :to="`/repo/${repoHash}/specs/${task.spec_ref.split('@')[0]}`"
            ...
            <div class="rounded-lg bg-white dark:bg-gray-900 p-5 shadow-card dark:ring-1 dark:ring-white/[0.06] dark:shadow-none">
              ...
              <DependencyTree :deps="task.deps_detail" :repo-hash="repoHash" />
            </div>
          </div>
