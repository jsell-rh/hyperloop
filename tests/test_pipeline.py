"""Tests for the pipeline executor — pure function that advances through pipeline steps.

The pipeline executor takes a pipeline definition, current position, and worker result,
and returns the next action to take and the new position.
"""
...
class TestSimpleSequence:
    ...
    def test_start_spawns_first_role(self):
        ...
        action, new_pos = next_action(self.pipeline, pos(0), result=None)
    ...
    def test_pass_advances_to_next_role(self):
        ...
        action, new_pos = next_action(self.pipeline, pos(0), result=PASS)
    ...
    def test_verifier_pass_advances_to_action(self):
        ...
        action, new_pos = next_action(self.pipeline, pos(1), result=PASS)
    ...
    def test_action_pass_completes_pipeline(self):
        ...
        action, _new_pos = next_action(self.pipeline, pos(2), result=PASS)
    ...
    def test_fail_outside_loop_fails_pipeline(self):
        ...
        action, _new_pos = next_action(self.pipeline, pos(1), result=FAIL)
...
class TestLoop:
    ...
    def test_start_enters_loop_at_first_step(self):
        ...
        action, new_pos = next_action(self.pipeline, pos(0, 0), result=None)
        assert action == SpawnRole(role="implementer")
...
class TestNestedLoops:
    ...
    def test_start_at_inner_implementer(self):
        action, new_pos = next_action(self.pipeline, pos(0, 0, 0), result=None)
        assert action == SpawnRole(role="implementer")
        assert new_pos == pos(0, 0, 0)

    def test_inner_implementer_pass_to_verifier(self):
        action, new_pos = next_action(self.pipeline, pos(0, 0, 0), result=PASS)
        assert action == SpawnRole(role="verifier")
    ...
    def test_inner_verifier_fail_restarts_inner_loop(self):
        ...
        action, new_pos = next_action(self.pipeline, pos(0, 0, 1), result=FAIL)
        assert action == SpawnRole(role="implementer")
        assert new_pos == pos(0, 0, 0)

    def test_inner_verifier_pass_exits_inner_advances_to_security(self):
        """Verifier pass exits inner loop, advances to security."""
        action, new_pos = next_action(self.pipeline, pos(0, 0, 1), result=PASS)
    ...
    def test_merge_pass_completes_pipeline(self):
        action, _new_pos = next_action(self.pipeline, pos(1), result=PASS)
        assert action == PipelineComplete()
...
class TestGate:
    ...
    def test_at_gate_no_result_returns_wait(self):
        ...
        action, new_pos = next_action(self.pipeline, pos(1), result=None)
        assert action == WaitForGate(gate="human-pr-approval")
        assert new_pos == pos(1)
...
class TestRoutingOverrides:
    ...
    def test_on_fail_override_jumps_to_named_role(self):
        ...
        action, new_pos = next_action(pipeline, pos(1), result=FAIL)
        assert action == SpawnRole(role="implementer")
        assert new_pos == pos(0)
...
class TestEdgeCases:
    def test_single_action_step(self):
        ...
        action, new_pos = next_action(pipeline, pos(0), result=None)
        assert action == PerformAction(action="merge-pr")
        assert new_pos == pos(0)
    ...
    def test_action_step_no_result_performs_action(self):
        ...
        pipeline = (ActionStep(action="merge-pr"),)
        action, _new_pos = next_action(pipeline, pos(0), result=None)
        assert action == PerformAction(action="merge-pr")
    ...
    def test_loop_with_single_role(self):
        ...
        action, new_pos = next_action(pipeline, pos(0, 0), result=FAIL)
        assert action == SpawnRole(role="implementer")
        assert new_pos == pos(0, 0)
    ...
    def test_position_into_loop_entry(self):
        ...
        action, new_pos = next_action(pipeline, pos(0, 0), result=None)
        assert action == SpawnRole(role="implementer")
        assert new_pos == pos(0, 0)
