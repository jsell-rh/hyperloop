...
class TestCreateDraft:
    def test_returns_url(self):
        ...
        url = pr.create_draft("task-001", "worker/task-001", "Implement widget", "specs/widget.md")
    ...
    def test_stores_branch_and_title(self):
        ...
        assert info["branch"] == "worker/task-001"
        assert info["title"] == "Implement widget"

    def test_spec_ref_label_derives_name_from_path(self):
        pr = FakePRManager(repo="org/repo")
        url = pr.create_draft(
            "task-027", "worker/task-027", "Implement persistence", "specs/persistence.md"
        )
        labels = pr.get_labels(url)
    ...
    def test_spec_ref_label_handles_nested_paths(self):
        pr = FakePRManager(repo="org/repo")
        url = pr.create_draft(
            "task-050", "worker/task-050", "Implement feature", "specs/sub/feature.md"
        )
        labels = pr.get_labels(url)
...
class TestCheckGate:
    def test_returns_true_when_lgtm_present(self):
        pr = FakePRManager(repo="org/repo")
        url = pr.create_draft("task-001", "worker/task-001", "Widget", "specs/widget.md")
...
class TestRebaseBranch:
    def test_rebase_succeeds_by_default(self):
        pr = FakePRManager(repo="org/repo")
        result = pr.rebase_branch("worker/task-001", "main")
        assert result is True

    def test_rebase_fails_when_configured(self):
        pr = FakePRManager(repo="org/repo")
        pr.set_rebase_fails("worker/task-001")
        result = pr.rebase_branch("worker/task-001", "main")
        assert result is False

    def test_rebase_records_call(self):
        pr = FakePRManager(repo="org/repo")
        pr.rebase_branch("worker/task-001", "main")
        assert ("worker/task-001", "main") in pr.rebased
