...
class FakeObserver:
    def __init__(self) -> None:
        self.calls: list[ObserverCall] = []

    def _record(self, method: str, **kwargs: object) -> None:
        self.calls.append(ObserverCall(method=method, kwargs=kwargs))

    def calls_for(self, method: str) -> list[dict[str, object]]:
        return [c.kwargs for c in self.calls if c.method == method]

    def reconciler_started(self, *, spec_count: int, cycle: int) -> None:
        self._record("reconciler_started", spec_count=spec_count, cycle=cycle)

    def reconciler_halted(self, *, reason: str, total_cycles: int) -> None:
        self._record("reconciler_halted", reason=reason, total_cycles=total_cycles)
    ...
    def decomposition_started(self, *, specs_count: int, cycle: int) -> None:
        self._record("decomposition_started", specs_count=specs_count, cycle=cycle)
    ...
    def decomposition_failed(self, *, reason: str, cycle: int) -> None:
        self._record("decomposition_failed", reason=reason, cycle=cycle)
    ...
    def agent_text(self, *, branch: str, text_preview: str) -> None:
        self._record("agent_text", branch=branch, text_preview=text_preview)

    def agent_progress(self, *, branch: str, description: str) -> None:
        self._record("agent_progress", branch=branch, description=description)

    def agent_error(self, *, branch: str, error: str) -> None:
        self._record("agent_error", branch=branch, error=error)
    ...
    def composer_rebuild_failed(self, *, reason: str) -> None:
        self._record("composer_rebuild_failed", reason=reason)

    def plan_synced(self, *, cycle: int) -> None:
        self._record("plan_synced", cycle=cycle)
